...
from analysis_settings import img_size_, dynamic_mask_lrs, \
    best_loss_, iteration_epoch_, iteration_epoch_min_, mask_optimizer_lr_, check_epoch_, \
...
def Prototype_CL(model_path, Prototype_path, Feature_region_t, prototype_num, prototype_class):
    ...
    each_saleincy_map_path = './' + prototype_class + '_feature_saliency_map'
    final_saleincy_map_path = './' + prototype_class + '_final_feature_saliency_map'

    prototype_path = Prototype_path
    original_img1 = cv2.imread(prototype_path)[..., ::-1]
    ...
    img_size = img_size_
    current_mask_img_root_path = './' + prototype_class + '_current_mask_img_root_path'
    ...
    if(prototype_class == 'Positive'):
        _, logits_index = torch.min(positive_square[0, 0:49, int(prototype_num)], 0)
    else:
        _, logits_index = torch.min(negative_square[0, 0:49, int(prototype_num)], 0)
    ...
    for base_mask_num in range(len(base_mask_size_list)):
        ...
        store_path = os.path.join(each_saleincy_map_path, str(base_mask_size_list[base_mask_num]), Feature_region_t, prototype_num)
        makedir(store_path)
    ...
    for size in size_list:
        feature_t_list = os.listdir(os.path.join(each_saleincy_map_path, size))
        for feature_t in feature_t_list:
            prototype_num_list = os.listdir(os.path.join(each_saleincy_map_path, size, feature_t))
            for prototype_t in prototype_num_list:
                each_saliency_map = cv2.imread(os.path.join(each_saleincy_map_path, size, feature_t, prototype_t, 'saliency_map.jpg'), cv2.IMREAD_GRAYSCALE)
                ...
                final_original_img = cv2.imread(os.path.join(each_saleincy_map_path, size, feature_t, prototype_t, 'original_img.jpg'))[..., ::-1]

    makedir(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num))
    ...
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'fianl_saliency.jpg'), sum_each_saliency)
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'original_img.jpg'), final_original_img)
    ...
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'cam.jpg'), final_cam)
    ...
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'final_crop_patch.jpg'), final_crop_patch)
    read_final_original_img = cv2.imread(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'original_img.jpg'))[..., ::-1]
    read_final_heatmap = cv2.imread(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'cam.jpg'))[..., ::-1]
    ...
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'crop_original_img.jpg'), crop_final_img)
    plt.imsave(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num, 'crop_cam.jpg'), crop_final_heatmap)
    ...
    print("prototype_saliency_map_path:")
    print(os.path.join(final_saleincy_map_path, Feature_region_t, prototype_num))